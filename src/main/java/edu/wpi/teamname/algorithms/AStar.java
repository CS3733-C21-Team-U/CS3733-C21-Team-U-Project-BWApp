...
public class AStar {
  ...
  public AStar(Graph graph) { // initial map for AStar
    this.map = graph;
  }

  public void updateAStar(Graph graph){ // for updating map
    this.map = graph;
  }
  public void run(node _startNode, node _goalNode) {
    ...
    HashMap<node, node> cameFrom = new HashMap<>();   //record what node leads to which for shortest path
    HashMap<node, Double> cost = new HashMap<>();     //record the cost to get to each node from the start node
    HashMap<node, Double> priority = new HashMap<>(); //priority the cost + euclidean distance to the goal
    ...
    while (!reachableNodes.isEmpty()) {
      ...
      for (node adjNode : current.adjNodes()) {
        ...
        //if the cost to get to the adjacent node is less update its cost and add it back to the reachable nodes
        else if (cost.get(adjNode) > nextNodeCost) {//this has to be in the else if because cost.containsKey can return NULL
          cost.put(adjNode, nextNodeCost);
          priority.put(adjNode, nextNodeCost + distBetweenNodes(adjNode, _goalNode));
          reachableNodes.add(adjNode);
          cameFrom.put(adjNode, current);
